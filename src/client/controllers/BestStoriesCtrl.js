...
(function () {
	angular.module('blog')
		.controller('BestStoriesCtrl',['getBlog','$stateParams',GetBestStories]);

	function GetBestStories(getBlog,$stateParams) {

		var vm = this;
		vm.hello = 'Best Stories';
		vm.bestStories = [];
		vm.comments = [];
		vm.submitted = [];
		...
		//Gets all the Best stories ID. And calls a getBlog.getNews function from the factory service passing the ID
		// and where the data needs to be pushed.
		getBlog.bestStories.query(function (data) {
			for (var i= 0; i < data.length; i++) {
				getBlog.getNews(data[i],vm.bestStories)
			}
