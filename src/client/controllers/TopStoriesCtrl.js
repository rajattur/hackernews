...
(function () {
	angular.module('blog')
		.controller('TopStoriesCtrl',['getBlog','$stateParams',GetTopStories]);

	function GetTopStories(getBlog,$stateParams) {

		var vm = this;
		vm.hello = 'Top Stories';
		vm.topStories = [];
		vm.comments = [];
		vm.submitted = [];
		vm.id = $stateParams.id || 1;
		vm.userName = $stateParams.userName;

		//Gets all the top stories ID. And calls a getBlog.getNews function from the factory service passing the ID
		// and where the data needs to be pushed.
		getBlog.topStory.query(function (data) {
			for (var i= 0; i < data.length; i++) {
				getBlog.getNews(data[i], vm.topStories)
			}
